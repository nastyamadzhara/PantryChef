...
function LogIn() {
    ...
    const logInOn = async () => {
        setError(''); // Очищаємо помилку перед логіном
        if (username && password) {
            try {
                ...
                if (response.ok) {
                    ...
                    navigate('/'); // Перенаправлення на головну сторінку після успішного логіну
                } else {
                    const data = await response.json();
    ...
    return (
        <div className="back">
            <div className="bigRectangleElement">
                ...
                <button className="button" onClick={logInOn}>Next</button>

                {error && <p className="errorMessage">{error}</p>} {/* Відображення помилки */}

                <div className="textBlock">
                    <p className="text2">
                        Don't have an account? <u onClick={handleClick}>Sign up</u>
                    </p>
                </div>
            </div>
        </div>
